...
# v13 - Added metrics
# v14 - Now the trajectory is saved as a separate point cloud from the static map
...
# Configuration
CONFIG = {
    "selected_file": "strada1", # Change this to the desired file: "capture1911", "strada1", "strada2", "strada3"
...
def save_fused_map(fused_static_cloud, timestamp, base_path, name):
    output_path = os.path.join(base_path, f"{timestamp}_fused_{name}_map.ply")
    o3d.io.write_point_cloud(output_path, fused_static_cloud)
...
def save_timestamp_and_parameters(timestamp, base_path, execution_time):
    ...
    # Append the timestamp and parameters to the file
...
if __name__ == "__main__":
    ...
    for i in range(0, min(len(lidar1["csv_files"]), len(lidar2["csv_files"]), CONFIG["max_number_of_clouds"])):
        ...
        if maps_are_aligned == True:
            ...
            lidar2_map_transformed += lidar2["maps"]["static"]
            ...
            fused_static_cloud = lidar1["maps"]["static"] + lidar2_map_transformed_filtered
            
            lidar2_trajectory_map_transformed = o3d.geometry.PointCloud()
            lidar2_trajectory_map_transformed += lidar2["maps"]["trajectory"]
            lidar2_trajectory_map_transformed.transform(lidar2_map_transformation)
            fused_trajectory_cloud = lidar1["maps"]["trajectory"] + lidar2_trajectory_map_transformed
            
            fused_static_trajectory_cloud = fused_static_cloud + fused_trajectory_cloud
            
            if CONFIG["print_realtime"]:
                display_point_cloud(vis_fusion, fused_static_trajectory_cloud, point_size=CONFIG["point_size"])
                print(f"Number of points in the fused map: {len(fused_static_trajectory_cloud.points)}")
    ...
    if CONFIG["save_map"]:
        ...
        if maps_are_aligned == True:
            save_fused_map(fused_static_cloud, timestamp, base_path, "static")
            save_fused_map(fused_trajectory_cloud, timestamp, base_path, "trajectory")
