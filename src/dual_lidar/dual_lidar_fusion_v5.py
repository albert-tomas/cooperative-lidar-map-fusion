...
def icp_alignment(csv_files, folder_path, i, input_cloud, maps, icp_params, current_transformation, max_count, model):
...
def process_frame(i, lidar, model):
    ...
    predictions, results = icp_alignment(
        ...
        lidar["current_transformation"], lidar["max_count"], model
    )
    ...
    if CONFIG["print_realtime"]:
        display_point_cloud(lidar["vis_map"], show_cloud, point_size=1.5)
...
if __name__ == "__main__":
    ...
    lidar1 = initialize_lidar()
    lidar2 = initialize_lidar()
    
    for i in range(0, min(len(lidar1["csv_files"]), len(lidar2["csv_files"]), CONFIG["max_number_of_clouds"])):
        
        print("Starting iteration: ", i)
        
        pred1 = process_frame(i, lidar1, model)
        pred2 = process_frame(i, lidar2, model)
    ...
    if CONFIG["save_map"]:
        ...
        save_static_map(lidar1["maps"]["static"], timestamp, base_path)
        save_trajectory(lidar1["maps"]["trajectory"], timestamp, base_path)
        if len(lidar1["maps"]["person"].points) <= 0:
            lidar1["maps"]["person"] = generate_single_point()
            change_colors(lidar1["maps"]["person"], COLORS["GREY"])
        save_person_cloud(lidar1["maps"]["person"], timestamp, base_path)
