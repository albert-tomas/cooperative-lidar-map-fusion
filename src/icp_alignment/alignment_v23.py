...
# v22 - Added saving maps every 1000 frames with metrics
# v23 - Added some extras to generate images for the presentation
...
# Configuration
CONFIG = {
    "selected_file": "strada2", # Change this to the desired file: capture1911, strada1, strada2, strada3
    "max_number_of_clouds": 500,
    "voxel_size": 0.1,
    "print_realtime": True,
    "save_map": False,
    "realtime_people_only": True, # If True, visualizes the dynamic objects (people) only in the current frame
...
def initialize_roboflow():
    # Initialize Roboflow
    rf = Roboflow(api_key="m3mVIcAJXOFxaob0eVAt") # Replace with your actual API key

    # Load the trained model
    model = rf.workspace("sicariata").project("person_lidar").version(1).model
    return model

def yolo_add_boxes(image_file, output_image_path, model, max_retries=1, retry_delay=0.5):
    ...
    for attempt in range(1, max_retries + 1):
        try:
            resp = model.predict(image, confidence=10, overlap=30)
            predictions = resp.json()
            break
        except requests.exceptions.RequestException as e:
            print(f"Roboflow request error (attempt {attempt}/{max_retries}): {e}")
        except ValueError as e:
            print(f"Roboflow response parse error (attempt {attempt}/{max_retries}): {e}")
        time.sleep(retry_delay)
    else:
        print(f"Error: Roboflow failed after {max_retries} attempts, skipping detection.")
        predictions = {"predictions": [], "image": {"width": 0, "height": 0}}
...
def add_to_static_map(static_map, new_cloud, counter, vsize, max_count):
    new_points = np.asarray(new_cloud.points)
    voxel_indices = np.floor(new_points / vsize).astype(int)
    ...
    # Create a new point cloud with colors for the new points
    pc = o3d.geometry.PointCloud()
    pc.points = o3d.utility.Vector3dVector(new_points)
    # pc.colors = o3d.utility.Vector3dVector(colors)

    static_map += pc
    return static_map, max_count

def data_to_csv(data, i, elapsed_ms, points_original, points_person, points_static, points_added, rmse, fitness):
    # Prepare the data to be saved
    data.append ({
        "frame": i,
        "elapsed_ms": elapsed_ms,
        "points_original": points_original,
        "points_person": points_person,
...
def icp_alignment(csv_files, folder_path, vis_map, output_dir):
    # Configuration
    ...
    # pre_vis = setup_visualizer()
    # after_vis = setup_visualizer()
    ...
    print(f"Loaded first cloud from {first_csv} with {len(input_cloud.points)} points.")
    ...
    show_cloud = static_map + trajectory_global_cloud + person_cloud

    # Display the initial point cloud
    if CONFIG["print_realtime"]:
        display_point_cloud(vis_map, trajectory_global_cloud, point_size=1.0)
    ...
    if CONFIG["save_map"]:
        metrics_data = []
        metrics_data = data_to_csv(metrics_data,0,elapsed_time,len(input_cloud.points),len(person_cloud.points),
                        len(static_map.points),len(static_map.points), 0, 1.0)
        df = pd.DataFrame(metrics_data)
        metrics_path = os.path.join(output_dir, "slam_metrics.csv")
    ...
    for i in range(1, min(len(csv_files), max_number_of_clouds)):
        ...
        # if i >= 50:
        #     pre_cloud = o3d.geometry.PointCloud()
        #     pre_cloud += current_cloud_downsampled + static_map
        #     display_point_cloud(pre_vis, pre_cloud, point_size=1.0)
        ...
        # if i >= 50:
        #     after_cloud = o3d.geometry.PointCloud()
        #     after_cloud += current_cloud_downsampled + static_map
        #     display_point_cloud(after_vis, after_cloud, point_size=1.0)
        ...
        static_map, max_count = add_to_static_map(static_map, filtered_cloud, voxel_counter, voxel_size, max_count)

        # if i >= 5:
        #     display_point_cloud(pre_vis, current_cloud_downsampled, point_size=1.0)
        #     print(f"Number of points in current cloud: {len(current_cloud_downsampled.points)}")
        #     display_point_cloud(after_vis, filtered_cloud, point_size=1.0)
        #     print(f"Number of points in filtered cloud: {len(filtered_cloud.points)}")
        #     pre_vis.run()
            # after_vis.run()
        ...
        if CONFIG["realtime_people_only"]:
            person_show = person_cloud
        else:
            person_show = person_global_cloud
        
        show_cloud = static_map + trajectory_global_cloud + person_show
        ...
        if CONFIG["save_map"] and i % 50 == 0:
            metrics_data = data_to_csv(metrics_data,i,elapsed_time,len(current_cloud.points),len(person_cloud.points),
                    len(static_map.points),len(filtered_cloud.points), icp_result.inlier_rmse, icp_result.fitness)
            df = pd.DataFrame(metrics_data)
            save_static_map(static_map, output_dir, i)
            save_trajectory(trajectory_global_cloud, output_dir, i)
            save_person_cloud(person_global_cloud, output_dir, i)
    ...
    return static_map, trajectory_global_cloud, person_global_cloud, i, df, metrics_path

def save_static_map(global_map, base_path, i):
    output_path = os.path.join(base_path, f"{i}_frames_static_map.ply")
    o3d.io.write_point_cloud(output_path, global_map)
    print(f"Static map saved to {output_path}")

def save_trajectory(global_trajectory, base_path, i):
    output_path = os.path.join(base_path, f"{i}_frames_trajectory_map.ply")
    o3d.io.write_point_cloud(output_path, global_trajectory)
    print(f"Trajectory map saved to {output_path}")

def save_person_cloud(person_global_cloud, base_path, i):
    output_path = os.path.join(base_path, f"{i}_frames_person_map.ply")
...
def main():
    ...
    if CONFIG["save_map"]:
        ...
        base_path = f"D:/LiDAR-captures/{CONFIG['selected_file']}/maps_for_visualization"
    ...
    # Run alignment
    static_map, global_trajectory, global_person_cloud, i, df, metrics_path = icp_alignment(csv_files, folder_path, vis_map, output_dir)
    
    # Save results if enabled
    if CONFIG["save_map"]:
        save_static_map(static_map, output_dir, i)
        save_trajectory(global_trajectory, output_dir, i)
        save_person_cloud(global_person_cloud, output_dir, i)
        df.to_csv(metrics_path, index=False, float_format='%.4f')
