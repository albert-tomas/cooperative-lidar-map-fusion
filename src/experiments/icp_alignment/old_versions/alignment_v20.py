...
def add_to_static_map(static_map, new_cloud, counter, vsize, max_count):
    pts = np.asarray(new_cloud.points)
    ix = np.floor(pts / vsize).astype(int)

    for idx in map(tuple, ix):
        counter[idx] += 1
...
def icp_alignment(csv_files, folder_path, vis_map):
    ...
    for i in range(1, min(len(csv_files), max_number_of_clouds)):
        ...
        static_map = add_to_static_map(static_map, filtered_cloud, voxel_counter, voxel_size, max_count)
