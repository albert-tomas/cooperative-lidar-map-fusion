...
# Configuration
CONFIG = {
    "selected_file": "capture1911", # Change this to the desired file
...
def icp_alignment(csv_files, folder_path, vis_map):
    ...
    for i in range(1, min(len(csv_files), max_number_of_clouds)):
        ...
        #     cloud += current_cloud + person_cloud
        #     display_point_cloud(vis_test, cloud, point_size=1.0)
        #     vis_test.run()
        ...
        if i== 188:
            print("Entering ICP alignment for cloud", i)
            vis_test = setup_visualizer()
            # cloud = o3d.geometry.PointCloud()
            change_colors(current_cloud_static, COLORS["GREY"])
            cloud = current_cloud_static + person_cloud
            display_point_cloud(vis_test, cloud, point_size=2.0)
            vis_test.run()
